...
namespace FS::VK
{
    ...
    {
        mLayout = device.CreatePipelineLayout(builder.mLayoutCreateInfo);
    ...
    void GraphicsPipeline::CreatePipeline(const GraphicsPipelineBuilder& builder, const Device& device)
    {
        ...
        auto [vertexStage, vertexModule] = device.CreateShaderStage(vk::ShaderStageFlagBits::eVertex,
                                                                    builder.mShaderPaths[0]);
        auto [fragmentStage, fragmentModule] = device.CreateShaderStage(vk::ShaderStageFlagBits::eFragment,
                                                                        builder.mShaderPaths[1]);
