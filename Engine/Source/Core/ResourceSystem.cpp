...
namespace FS
{
    ...
    std::optional<Model> ResourceSystem::LoadGLTF(const std::filesystem::path& path)
    {
        ...
        {
            // Log the error
            Log::Error("Error while parsing gltf file {}, {}", path.string(), fastgltf::getErrorMessage(expectedAsset.error()));
            return std::nullopt;
        }
        ...
        model.mRootNodes.assign(assetNodes.begin(), assetNodes.end());
        ...
        model.mTextures.reserve(asset.images.size());
        ExtractGltfMeshes(model, asset);
        ExtractGltfTextures(model, asset);
        ExtractGltfMaterials(model, asset);
                ...
                if (primitive.indicesAccessor.has_value())
                {
                    ...
                    switch (indexAccessor.componentType)
                    {
                        case fastgltf::ComponentType::UnsignedShort:
                            ...
                            std::ranges::transform(shortIndices,
                                                   indices.begin(),
    ...
    void ResourceSystem::ExtractGltfMaterials(Model& model, const fastgltf::Asset& asset)
    {
        for (const auto& material : asset.materials)
        {
            const auto& [fastBaseColorFactor, metallicFactor, roughnessFactor, baseColorTexture, metallicRoughnessTexture] =
                material.pbrData;
    ...
    {
        return std::move(mModelsToUpload);
    }
    ...
    void ResourceSystem::ExtractGltfTextures(Model& model, const fastgltf::Asset& asset)
    {
        for (const auto& [source, name] : asset.images)
        {
            ...
            std::visit(
                ...
                source);
            model.mTextures.emplace_back(width, height, pixels);
        }
    }

    void ResourceSystem::ExtractGltfNodes(Model& model, const fastgltf::Asset& asset)
    {
